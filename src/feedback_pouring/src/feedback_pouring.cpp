...
{
    std::cout << "Finding the mass of the gripper" << std::endl;
    ...
    while (1)
    {
        ...
        std::cout << "Inital Force_x: " << initial_wrench.force.x() << " Force_y: " << initial_wrench.force.y() << " Force_z: " << initial_wrench.force.z() << std::endl;
...
{
    std::cout << "Controlling the End Effector" << std::endl;
    ...
    {   
        ...
        // auto mass = ( (force/acceleration) - (gripper_mass + 0.00));
        ...
        if (!target_mass_set) {
            ...
            if (mass_extract_previous != mass_extract_current ){
                ...
                std::cout << "Target Mass is set to: " << target_mass << std::endl;
        ...
        if(target_mass)
        {   
            if (mass > (target_mass + 0.1 * target_mass))
            {
                decremental_speed = SPEED/i;
                std::cout << "Decremental Speed: " << decremental_speed << std::endl;
                std :: cout << "i_value: " << i << std::endl;
            ...
            else if (mass < (target_mass + 0.1 * target_mass))
            {   
                while (1)
                {
                    ...
                    // std :: cout << "Last Actuator Position: " << RAD_TO_DEG(jnt_pos_stop.data(6)) << std::endl;
        ...
        //     for (int i = 0; i < chain.getNrOfJoints(); i++)
        //     { 
        //         std::cout << "Joint " << i << " Position: " << RAD_TO_DEG(jnt_pos.data(i)) << std::endl;
        //     }

        // }
        // else if (i >= 100)
        // {
        //     // std :: cout << "Last Actuator Position: " << RAD_TO_DEG(jnt_pos.data(6)) << std::endl;
...
int main(int argc, char **argv)
{
    ...
    // Example core
    bool success = true;
    double gripper_pallavi = 0.0f;
    success &= example_move_to_home_position(base);
    success &= move_to_cartesian_position(base, targetPose, base_cyclic);
    // gripper_pallavi =  find_gripper_mass(base, base_cyclic, interconnect_cyclic);
    std::cout << "Gripper Mass returned: " << gripper_pallavi << std::endl;
